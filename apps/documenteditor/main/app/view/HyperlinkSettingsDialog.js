...
define([
  ...
  DE.Views.HyperlinkSettingsDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 350,
        ...
        initialize: function (options) {
          ...
          this.template = [
            ...
            '<button type="button" class="btn btn-text-default auto" id="id-dlg-hyperlink-external">',
            this.textExternal,
            "</button>",
            '<button type="button" class="btn btn-text-default auto" id="id-dlg-hyperlink-internal">',
            this.textInternal,
            "</button>",
            "</div>",
        ...
        render: function () {
          ...
          me.btnExternal.on(
            "click",
            _.bind(me.onLinkTypeClick, me, c_oHyperlinkType.WebLink)
          );
            ...
            if (store.length < 1) {
              ...
              for (var i = 0; i < count; i++) {
                var anchor = anchors[i],
                  hasParent = true;
                if (
                  anchor.asc_GetType() == Asc.c_oAscHyperlinkAnchor.Bookmark
                ) {
                  if (prev_level < 1)
                    arr[arr.length - 1].set("hasSubItems", true);
                  arr.push(
          ...
          if (this.isAutoUpdate) {
            if (type == c_oHyperlinkType.InternalLink) {
              ...
              this.inputDisplay.setValue(
                rec && (rec.get("level") || rec.get("index") == 0)
                  ? rec.get("name")
                  : ""
              );
            } else {
          ...
          if (props) {
            ...
            var bookmark = props.get_Bookmark(),
              type =
                bookmark === null || bookmark == ""
                  ? props.get_Value() ||
                    !Common.Utils.InternalSettings.get("de-settings-link-type")
                    ? c_oHyperlinkType.WebLink
                    : c_oHyperlinkType.InternalLink
                  : c_oHyperlinkType.InternalLink;

            type == c_oHyperlinkType.WebLink
              ? me.btnExternal.toggle(true)
              : me.btnInternal.toggle(true);
            ...
            if (type == c_oHyperlinkType.WebLink) {
              ...
              if (props.is_TopOfDocument()) this.internalList.selectByIndex(0);
                ...
                var rec = this.internalList.store.findWhere({
                  type: Asc.c_oAscHyperlinkAnchor.Heading,
                  headingParagraph: props.get_Heading(),
                });
                if (rec) {
                  this.internalList.expandRecord(this.internalList.store.at(1));
                  this.internalList.scrollToRecord(
                    this.internalList.selectRecord(rec)
                  );
                }
              } else {
                var rec = this.internalList.store.findWhere({
                ...
                if (rec) {
                  this.internalList.expandRecord(
                    this.internalList.store.findWhere({
                      type: Asc.c_oAscHyperlinkAnchor.Bookmark,
                      level: 0,
                    })
                  );
        ...
        _handleInput: async function (state) {
          if (this.options.handler) {
            if (state == "ok") {
              ...
              !this._originalProps.get_Bookmark() &&
                !this._originalProps.get_Value() &&
