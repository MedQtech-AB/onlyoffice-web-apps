...
define([
  ...
  DE.Controllers.Main = Backbone.Controller.extend(
    _.extend(
      (function () {
        var appHeader;
        ...
        return {
          ...
          initialize: function () {
            ...
            styleNames.forEach(function (item) {
              translate[item] =
                me["txtStyle_" + item.replace(/ /g, "_")] || item;
            });
          ...
          onLaunch: function () {
            ...
            if (!Common.Utils.isBrowserSupported()) {
              ...
              Common.Gateway.reportError(
                undefined,
                this.unsupportedBrowserErrorText
              );
              return;
            }
            ...
            if (this.api) {
              ...
              var value = Common.localStorage.getBool(
                "de-settings-cachemode",
                true
              );
              ...
              this.api.asc_registerCallback(
                "asc_onDocumentName",
                _.bind(this.onDocumentName, this)
              );
              this.api.asc_registerCallback(
                "asc_onPrintUrl",
                _.bind(this.onPrintUrl, this)
              );
              this.api.asc_registerCallback(
                "asc_onMeta",
                _.bind(this.onMeta, this)
              );
              this.api.asc_registerCallback(
                "asc_onSpellCheckInit",
                _.bind(this.loadLanguages, this)
              );
              ...
              Common.NotificationCenter.on(
                "showmessage",
                _.bind(this.onExternalMessage, this)
              );
              Common.NotificationCenter.on(
                "showerror",
                _.bind(this.onError, this)
              ...
              Common.NotificationCenter.on(
                "editing:disable",
                _.bind(this.onEditingDisable, this)
              );
              ...
              /** coauthoring begin **/
              this.contComments = this.getApplication().getController(
              ...
              $(document.body)
                  ...
                  if (!Common.Utils.ModalWindow.isVisible()) {
                    ...
                    if (
                      ...
                      /* Check if focus goes to textarea, but not to "area_id" */ if (
                        ...
                        return;
                      if (
                        Common.Utils.isLinux &&
                        me.appOptions &&
                        me.appOptions.isDesktopApp
                      ) {
                        if (
                          e.relatedTarget ||
                          !e.originalEvent ||
                      ...
                      } else me.api.asc_enableKeyEvents(true);
                      if (
                        me.dontCloseDummyComment &&
                        /msg-reply/.test(e.target.className)
                      ) {
                        if (
                          $(e.target)
                            .closest(".user-comment-item")
                            .find(e.relatedTarget).length < 1
                        )
                          /* Check if focus goes to buttons in the comment window */
                          me.dontCloseDummyComment =
                            me.beforeCloseDummyComment = false;
                        else me.beforeCloseDummyComment = true;
                      } else if (/textarea-control/.test(e.target.className))
                ...
                .on("dragover", function (e) {
                  ...
                  if (
                    event.target &&
                    $(event.target).closest("#editor_sdk").length < 1
                  ) {
              ...
              Common.util.Shortcuts.delegateShortcuts({
                shortcuts: {
                  "command+s,ctrl+s,command+p,ctrl+p,command+k,ctrl+k,command+d,ctrl+d":
                    _.bind(function (e) {
                      e.preventDefault();
                      e.stopPropagation();
                    }, this),
            ...
            me.warnNoLicense = me.warnNoLicense.replace(
              /%1/g,
              "{{COMPANY_NAME}}"
            );
            me.warnNoLicenseUsers = me.warnNoLicenseUsers.replace(
              /%1/g,
              "{{COMPANY_NAME}}"
            );
            me.textNoLicenseTitle = me.textNoLicenseTitle.replace(
              /%1/g,
              "{{COMPANY_NAME}}"
            );
            me.warnLicenseExceeded = me.warnLicenseExceeded.replace(
              /%1/g,
              "{{COMPANY_NAME}}"
            );
            me.warnLicenseUsersExceeded = me.warnLicenseUsersExceeded.replace(
              /%1/g,
              "{{COMPANY_NAME}}"
            );
          },
          ...
          loadConfig: function (data) {
            ...
            this.appOptions.guestName =
              typeof this.appOptions.customization == "object" &&
              typeof this.appOptions.customization.anonymous == "object" &&
              typeof this.appOptions.customization.anonymous.label ==
                "string" &&
              this.appOptions.customization.anonymous.label.trim() !== ""
                ? Common.Utils.String.htmlEncode(
                    this.appOptions.customization.anonymous.label
                  )
                : this.textGuest;
            var value;
            ...
            this.editorConfig.user = this.appOptions.user =
              Common.Utils.fillUserInfo(
                this.editorConfig.user,
                this.editorConfig.lang,
                value
                  ? value + " (" + this.appOptions.guestName + ")"
                  : this.textAnonymous,
            ...
            Common.localStorage.setItem(
              ...
              this.editorConfig.getDocumentLinkToTypes
            );
            Common.localStorage.setItem(
              "getDocumentActionTypes",
              this.editorConfig.getDocumentActionTypes
            );
            ...
            this.appOptions.canOpenRecent =
              this.editorConfig.recent !== undefined;
            this.appOptions.templates = this.editorConfig.templates;
            this.appOptions.recent = this.editorConfig.recent;
            this.appOptions.createUrl = this.editorConfig.createUrl;
            this.appOptions.canRequestCreateNew =
              this.editorConfig.canRequestCreateNew;
            ...
            this.appOptions.location =
              typeof this.editorConfig.location == "string"
                ...
                : "";
            this.appOptions.sharingSettingsUrl =
              this.editorConfig.sharingSettingsUrl;
            ...
            this.appOptions.canBackToFolder =
              ...
              typeof this.editorConfig.customization.goback == "object" &&
            ...
            this.appOptions.canMakeActionLink =
              this.editorConfig.canMakeActionLink;
            this.appOptions.canRequestUsers = this.editorConfig.canRequestUsers;
            this.appOptions.canRequestSendNotify =
              this.editorConfig.canRequestSendNotify;
            this.appOptions.canRequestSaveAs =
              this.editorConfig.canRequestSaveAs;
            this.appOptions.canRequestInsertImage =
              this.editorConfig.canRequestInsertImage;
            this.appOptions.canRequestCompareFile =
              this.editorConfig.canRequestCompareFile;
            this.appOptions.canRequestMailMergeRecipients =
              this.editorConfig.canRequestMailMergeRecipients;
            this.appOptions.canRequestSharingSettings =
              this.editorConfig.canRequestSharingSettings;
            this.appOptions.canRequestSelectDocument =
              this.editorConfig.canRequestSelectDocument;
            this.appOptions.canRequestSelectSpreadsheet =
              this.editorConfig.canRequestSelectSpreadsheet;
            this.appOptions.compatibleFeatures =
              typeof this.appOptions.customization == "object" &&
              !!this.appOptions.customization.compatibleFeatures;
            this.appOptions.canFeatureComparison = true;
            this.appOptions.canFeatureContentControl = true;
            this.appOptions.canFeatureForms =
              !!this.api.asc_isSupportFeature("forms");
            this.appOptions.uiRtl = false;
            this.appOptions.disableNetworkFunctionality = !!(
              window["AscDesktopEditor"] &&
              window["AscDesktopEditor"]["isSupportNetworkFunctionality"] &&
              false ===
                window["AscDesktopEditor"]["isSupportNetworkFunctionality"]()
            );
            ...
            this.appOptions.user.guest &&
              ...
              Common.NotificationCenter.on(
                "user:rename",
                _.bind(this.showRenameUserDialog, this)
              );
            ...
            appHeader.setCanBack(
              this.appOptions.canBackToFolder === true,
              this.appOptions.canBackToFolder
            ...
            if (
              this.appOptions.location == "us" ||
              this.appOptions.location == "ca"
            )
              Common.Utils.Metric.setDefaultMetric(
                Common.Utils.Metric.c_MetricUnits.inch
              );

            value = Common.localStorage.getItem("de-macros-mode");
            if (value === null) {
              value = this.editorConfig.customization
                ? this.editorConfig.customization.macrosMode
                : "warn";
          ...
          loadDocument: function (data) {
            ...
            if (data.doc) {
              ...
              var coEditMode =
                type && typeof type[1] === "string"
                  ? "strict" // offline viewer for pdf|djvu|xps|oxps
                  : !(
                  ...
                  ? "fast" // if can change mode in viewer - set fast for using live viewer
            ...
            if (
              ...
              $("#editor-container").append(
                '<div class="doc-placeholder">' +
                  '<div class="line"></div>'.repeat(20) +
                  "</div>"
              );
              if (
                this.editorConfig.mode == "view" ||
                (this.permissions.edit === false && !this.permissions.review)
              )
                $("#editor-container")
                  .find(".doc-placeholder")
                  .css("margin-top", 19);
            }

            var type = data.doc
              ? /^(?:(docxf))$/.exec(data.doc.fileType)
              : false;
            this.appOptions.isFormCreator =
              !!(type && typeof type[1] === "string") &&
              this.appOptions.canFeatureForms; // show forms only for docxf
            ...
            this.api.asc_registerCallback(
              "asc_onMacrosPermissionRequest",
              _.bind(this.onMacrosPermissionRequest, this)
            );
            this.api.asc_registerCallback(
          ...
          onProcessSaveResult: function (data) {
            ...
            if (data && data.result === false) {
              Common.UI.error({
                ...
                msg: _.isEmpty(data.message)
                  ? this.errorProcessSaveResult
                  : data.message,
              });
            }
          },
          ...
          onDownloadAs: function (format) {
            if (
              !this.appOptions.canDownload &&
              !this.appOptions.canDownloadOrigin
            ) {
              Common.Gateway.reportError(
                Asc.c_oAscError.ID.AccessDeny,
                this.errorAccessDeny
              );
              return;
            }
            ...
            var _format =
                format && typeof format == "string"
                  ? Asc.c_oAscFileType[format.toUpperCase()]
                  : null,
              _defaultFormat = null,
              textParams,
              _supported = [
            ...
            if (type && typeof type[1] === "string") {
              if (
                !(format && typeof format == "string") ||
                type[1] === format.toLowerCase()
              ) {
                this.api.asc_DownloadOrigin(true);
                return;
              }
              if (/^xps|oxps$/.test(this.document.fileType))
                _supported = _supported.concat([
              ...
              textParams = new AscCommon.asc_CTextParams(
                Asc.c_oAscTextAssociation.PlainLine
              );
            } else {
              _supported = _supported.concat([
                Asc.c_oAscFileType.PDF,
            ...
            if (
              this.appOptions.canFeatureForms &&
              !/^djvu$/.test(this.document.fileType)
            ) {
              _supported = _supported.concat([
          ...
          onRefreshHistory: function (opts) {
            ...
            if (opts.data.error || !opts.data.history) {
              var historyStore = this.getApplication().getCollection(
              ...
              var versions = opts.data.history,
                historyStore = this.getApplication().getCollection(
                  "Common.Collections.HistoryVersions"
                ),
                currentVersion = null;
              if (historyStore) {
                var arrVersions = [],
                  ...
                  arrColors = [],
                  docIdPrev = "",
                  usersStore = this.getApplication().getCollection(
                    "Common.Collections.HistoryUsers"
                  ),
                  user = null,
                  usersCnt = 0;
                ...
                for (ver = versions.length - 1; ver >= 0; ver--) {
                  ...
                  if (
                    version.versionGroup === undefined ||
                  ...
                  if (version) {
                    ...
                    if (!user) {
                      user = new Common.Models.User({
                        ...
                        color: this.generateUserColor(
                          Asc.c_oAscArrUserColors[usersCnt++]
                        ),
                      });
                      usersStore.add(user);
                    }
                    ...
                    if (prev_ver !== version.version) {
                      ...
                      for (i = 0; i < arrColors.length; i++) {
                        arrVersions[arrVersions.length - i - 2].set(
                          "arrColors",
                          arrColors
                        );
                      }
                ...
                if (arrColors.length > 0) {
                  arrColors.reverse();
                  for (i = 0; i < arrColors.length; i++) {
                    arrVersions[arrVersions.length - i - 1].set(
                      "arrColors",
                      arrColors
                    );
                  }
          ...
          onEditingDisable: function (disable, options, type) {
            ...
            var prev_options =
              !disable && this.stackDisableActions.length() > 0
                ? this.stackDisableActions.get(
                    this.stackDisableActions.length() - 1
                  )
                : null;
            ...
            if (options.documentHolder) {
              options.documentHolder.clear &&
                app.getController("DocumentHolder").clearSelection();
              options.documentHolder.disable &&
                app
                  .getController("DocumentHolder")
                  .SetDisabled(
                    disable,
                    options.allowProtect,
                    options.fillFormMode
                  );
            }
            if (options.leftMenu) {
            ...
            if (options.comments) {
              var comments = this.getApplication().getController(
                "Common.Controllers.Comments"
              );
            ...
            if (prev_options) {
              this.onEditingDisable(
                prev_options.disable,
            ...
            if (this.api.isDocumentModified()) {
              ...
              Common.UI.warning({
                ...
                callback: function (btn) {
                  ...
                  } else me.api.asc_continueSaving();
                },
              });
            } else {
              Common.Gateway.requestClose();
          ...
          onEditComplete: function (cmp) {
            ...
            if (
              ...
              toolbarView &&
              (toolbarView.btnInsertShape.pressed ||
                toolbarView.btnInsertText.pressed) &&
              (!_.isObject(arguments[1]) ||
                arguments[1].id !== "tlbtn-insertshape")
            ) {
              // TODO: Event from api is needed to clear btnInsertShape state
              if (this.api) this.api.StartAddShape("", false);
            ...
            if (
              this.appOptions.isEdit &&
              toolbarView &&
              toolbarView.btnHighlightColor.pressed &&
              (!_.isObject(arguments[1]) ||
            ...
            if (
              this.appOptions.isEdit &&
              ((toolbarView && toolbarView._isEyedropperStart) ||
                (rightMenu && rightMenu._isEyedropperStart))
            ) {
              toolbarView._isEyedropperStart
                ? (toolbarView._isEyedropperStart = false)
                : (rightMenu._isEyedropperStart = false);
            ...
            if (
              ...
              var cansave = this.api.asc_isDocumentCanSave(),
                ...
                isSyncButton = toolbarView.btnCollabChanges.rendered
                  ? toolbarView.btnCollabChanges.cmpEl.hasClass("notify")
                  : false,
                isDisabled =
                  (!cansave && !isSyncButton && !forcesave) ||
                  this._state.isDisconnected ||
                  (this._state.fastCoauth &&
                    this._state.usersCount > 1 &&
                    !forcesave);
          ...
          onLongActionEnd: function (type, id) {
            ...
            action = this.stackLongActions.get({
              type: Asc.c_oAscAsyncActionType.Information,
            });
            if (action) {
              this.setLongActionView(action);
            } else {
              ...
              if (
                ...
                if (this._state.fastCoauth && this._state.usersCount > 1) {
                  me._state.timerSave = setTimeout(function () {
                    me.getApplication()
                      .getController("Statusbar")
                      .setStatusCaption(me.textChangesSaved, false, 3000);
                  }, 500);
                } else
                  me.getApplication()
                    .getController("Statusbar")
            ...
            if (
              ...
              (id == Asc.c_oAscAsyncAction["Save"] ||
                id == Asc.c_oAscAsyncAction["ForceSaveButton"]) &&
              (!this._state.fastCoauth ||
                this._state.usersCount < 2 ||
                this.getApplication()
                  .getController("Common.Controllers.ReviewChanges")
                  .isPreviewChangesMode())
            )
            ...
            if (id == Asc.c_oAscAsyncAction["Open"]) {
              Common.Utils.InternalSettings.get("de-settings-livecomment")
                ? this.api.asc_showComments(
                    Common.Utils.InternalSettings.get(
            ...
            if (id == Asc.c_oAscAsyncAction["Disconnect"]) {
              this._state.timerDisconnect &&
                clearTimeout(this._state.timerDisconnect);
              ...
              this.getApplication()
                .getController("Statusbar")
                .setStatusCaption(this.textReconnect);
            }
          ...
          onDocumentContentReady: function () {
            ...
            Common.Utils.InternalSettings.set(
              "de-settings-datetime-default",
              Common.localStorage.getItem("de-settings-datetime-default")
            );

            /** coauthoring begin **/
            this.isLiveCommenting = Common.localStorage.getBool(
              "de-settings-livecomment",
              true
            );
            Common.Utils.InternalSettings.set(
              "de-settings-livecomment",
              this.isLiveCommenting
            );
            ...
            this.isLiveCommenting
              ? this.api.asc_showComments(value)
              : this.api.asc_hideComments();
            /** coauthoring end **/
            ...
            var zf =
              value !== null
                ...
                ? parseInt(this.appOptions.customization.zoom)
                : 100;
            ...
            zf == -1
              ...
              : this.api.zoom(
                  zf > 0 ? zf : zf == -3 && lastZoom > 0 ? lastZoom : 100
                );
            ...
            value =
              value !== undefined
                ...
                : !(
                    this.appOptions.customization &&
                    this.appOptions.customization.spellcheck === false
                  );
            if (
              this.appOptions.customization &&
              this.appOptions.customization.spellcheck !== undefined
            )
              console.log(
                "Obsolete: The 'spellcheck' parameter of the 'customization' section is deprecated. Please use 'spellcheck' parameter in the 'customization.features' section instead."
              );
            if (Common.UI.FeaturesManager.canChange("spellcheck")) {
              // get from local storage
              value = Common.localStorage.getBool(
            ...
            if (Common.UI.FeaturesManager.canChange("spellcheck")) {
              // get settings for spellcheck from local storage
              value = Common.localStorage.getBool(
                "de-spellcheck-ignore-uppercase-words",
                true
              );
              Common.Utils.InternalSettings.set(
                "de-spellcheck-ignore-uppercase-words",
              ...
              value = Common.localStorage.getBool(
                "de-spellcheck-ignore-numbers-words",
                true
              );
              Common.Utils.InternalSettings.set(
                "de-spellcheck-ignore-numbers-words",
              ...
              value.put_IgnoreWordsInUppercase(
                Common.Utils.InternalSettings.get(
                  "de-spellcheck-ignore-uppercase-words"
                )
              );
              value.put_IgnoreWordsWithNumbers(
                Common.Utils.InternalSettings.get(
                  "de-spellcheck-ignore-numbers-words"
                )
              );
            ...
            me.api.asc_registerCallback(
              "asc_onStartAction",
              _.bind(me.onLongActionBegin, me)
            );
            me.api.asc_registerCallback(
              "asc_onEndAction",
              _.bind(me.onLongActionEnd, me)
            );
            me.api.asc_registerCallback(
            ...
            /** coauthoring begin **/
            me._state.fastCoauth = Common.Utils.InternalSettings.get(
              "de-settings-coauthmode"
            );
            me.api.asc_SetFastCollaborative(me._state.fastCoauth);

            value = Common.Utils.InternalSettings.get(
              me._state.fastCoauth
            ...
            me.api.asc_setAutoSaveGap(
              Common.Utils.InternalSettings.get("de-settings-autosave")
            );
            me.api.asc_SetPerformContentControlActionByClick(
              me.appOptions.isRestrictedEdit && me.appOptions.canFillForms
            );
            ...
            var toolbarController = application.getController("Toolbar"),
              ...
              documentHolderController =
                application.getController("DocumentHolder"),
              fontsController = application.getController(
                "Common.Controllers.Fonts"
              ),
              rightmenuController = application.getController("RightMenu"),
              leftmenuController = application.getController("LeftMenu"),
              chatController = application.getController(
                "Common.Controllers.Chat"
              ),
              pluginsController = application.getController(
            ...
            application
              .getController("Common.Controllers.ExternalMergeEditor")
              .setApi(this.api)
              .loadConfig({
                config: this.editorConfig,
                customization: this.editorConfig.customization,
            ...
            documentHolderController
              .getView()
              .on("editcomplete", _.bind(me.onEditComplete, me));

            if (me.appOptions.isEdit) {
              if (me.appOptions.canForcesave) {
              ...
              Common.Utils.InternalSettings.set(
                "de-settings-paste-button",
                parseInt(value)
              );
              me.api.asc_setVisiblePasteButton(!!parseInt(value));

              value = Common.localStorage.getBool(
                "de-settings-smart-selection",
                true
              );
              Common.Utils.InternalSettings.set(
                "de-settings-smart-selection",
              ...
              var timer_sl = setInterval(function () {
                if (window.styles_loaded) {
                  ...
                  if (shapes) me.fillAutoShapes(shapes[0], shapes[1]);
            ...
            setTimeout(function () {
              $(
                Common.Utils.String.format(".toolbar .lazy-{0}", dummyClass)
              ).remove();
            }, 10);

            if (this.appOptions.canAnalytics && false)
              Common.component.Analytics.initialize(
                "UA-12442749-13",
                "Document Editor"
              );
            ...
            Common.Gateway.on(
              "applyeditrights",
              _.bind(me.onApplyEditRights, me)
            );
            Common.Gateway.on(
              "processsaveresult",
              _.bind(me.onProcessSaveResult, me)
            );
            Common.Gateway.on(
              "processrightschange",
              _.bind(me.onProcessRightsChange, me)
            );
            Common.Gateway.on("processmouse", _.bind(me.onProcessMouse, me));
            Common.Gateway.on(
              "refreshhistory",
              _.bind(me.onRefreshHistory, me)
            );
            ...
            if (
              ...
              (licType === Asc.c_oLicenseResult.Connections ||
                ...
                (licType === Asc.c_oLicenseResult.SuccessLimit &&
                  (this.appOptions.trialMode & Asc.c_oLicenseMode.Limited) !==
                    0))
            )
          ...
          applyLicense: function () {
            if (this.editorConfig.mode === "view") {
              if (
                this.appOptions.canLiveView &&
                (this._state.licenseType ===
                  Asc.c_oLicenseResult.ConnectionsLive ||
                  this._state.licenseType ===
                    Asc.c_oLicenseResult.ConnectionsLiveOS ||
                  this._state.licenseType ===
                    Asc.c_oLicenseResult.UsersViewCount ||
                  this._state.licenseType ===
                    Asc.c_oLicenseResult.UsersViewCountOS ||
                  (!this.appOptions.isAnonymousSupport &&
                    !!this.appOptions.user.anonymous))
              ) {
              ...
              if (
                ...
                (license === Asc.c_oLicenseResult.SuccessLimit ||
                  this.appOptions.permissionsLicense ===
                    Asc.c_oLicenseResult.SuccessLimit)
              ) {
                ...
                license =
                  license === Asc.c_oLicenseResult.Connections
                    ? this.warnLicenseExceeded
                    : this.warnLicenseUsersExceeded;
              } else {
                license =
          ...
          onOpenDocument: function (progress) {
            ...
            proc =
              this.textLoadingDocument +
              ": " +
              Common.Utils.String.fixedDigits(
                Math.min(Math.round(proc * 100), 100),
                3,
                "  "
              ) +
              "%";
            elem
              ? (elem.innerHTML = proc)
              : this.loadMask && this.loadMask.setTitle(proc);
          },
          ...
          onEditorPermissions: function (params) {
            ...
            if (
              this.onServerVersion(params.asc_getBuildVersion()) ||
              !this.onLanguageLoaded()
            )
              return;

            var isPDFViewer = /^(?:(pdf|djvu|xps|oxps))$/.test(
              this.document.fileType
            );

            this.permissions.review =
              this.permissions.review === undefined
            ...
            this.appOptions.canCreateNew =
              this.appOptions.canCreateNew &&
              !(this.appOptions.isOffline && isPDFViewer);
            this.appOptions.isCrypted = this.api.asc_isCrypto();
            this.appOptions.isReviewOnly =
              this.permissions.review === true &&
              this.permissions.edit === false;
            this.appOptions.canRequestEditRights =
              this.editorConfig.canRequestEditRights;
            this.appOptions.canEdit =
              (this.permissions.edit !== false ||
                this.permissions.review === true) && // can edit or review
              (this.editorConfig.canRequestEditRights ||
                this.editorConfig.mode !== "view") && // if mode=="view" -> canRequestEditRights must be defined
            ...
            this.appOptions.canHistoryRestore =
              this.editorConfig.canHistoryRestore;
            this.appOptions.canUseMailMerge =
              this.appOptions.canLicense &&
              this.appOptions.canEdit &&
              !this.appOptions.isOffline;
            this.appOptions.canSendEmailAddresses =
              this.appOptions.canLicense &&
              this.editorConfig.canSendEmailAddresses &&
              this.appOptions.canEdit &&
              this.appOptions.canCoAuthoring;
            ...
            this.appOptions.canEditStyles =
              this.appOptions.canLicense && this.appOptions.canEdit;
            this.appOptions.canPrint = this.permissions.print !== false;
            this.appOptions.canPreviewPrint =
              this.appOptions.canPrint &&
              !Common.Utils.isMac &&
              this.appOptions.isDesktopApp;
            ...
            this.appOptions.canEditComments =
              this.appOptions.isOffline ||
              !this.permissions.editCommentAuthorOnly;
            this.appOptions.canDeleteComments =
              this.appOptions.isOffline ||
              !this.permissions.deleteCommentAuthorOnly;
            if (
              typeof this.editorConfig.customization == "object" &&
              ...
              if (
                ...
                this.appOptions.canEditComments =
                  this.appOptions.canDeleteComments = this.appOptions.isOffline;
            }
            if (typeof this.editorConfig.customization == "object") {
              if (
                this.editorConfig.customization.showReviewChanges !== undefined
              )
                console.log(
                  "Obsolete: The 'showReviewChanges' parameter of the 'customization' section is deprecated. Please use 'showReviewChanges' parameter in the 'customization.review' section instead."
                );
              if (this.editorConfig.customization.reviewDisplay !== undefined)
                console.log(
                  "Obsolete: The 'reviewDisplay' parameter of the 'customization' section is deprecated. Please use 'reviewDisplay' parameter in the 'customization.review' section instead."
                );
              if (this.editorConfig.customization.trackChanges !== undefined)
                console.log(
                  "Obsolete: The 'trackChanges' parameter of the 'customization' section is deprecated. Please use 'trackChanges' parameter in the 'customization.review' section instead."
                );
            }
            ...
            this.appOptions.canEditContentControl =
              this.permissions.modifyContentControl !== false;
            this.appOptions.canHelp = !(
              typeof this.editorConfig.customization == "object" &&
              this.editorConfig.customization.help === false
            );
            ...
            this.appOptions.canFillForms =
              this.appOptions.canLicense &&
              (this.appOptions.isEdit ? true : this.permissions.fillForms) &&
              this.editorConfig.mode !== "view";
            this.appOptions.isRestrictedEdit =
              !this.appOptions.isEdit &&
              (this.appOptions.canComments || this.appOptions.canFillForms);
            if (
              this.appOptions.isRestrictedEdit &&
              this.appOptions.canComments &&
              this.appOptions.canFillForms
            )
              // must be one restricted mode, priority for filling forms
            ...
            this.appOptions.canUseSelectHandTools =
              this.appOptions.canUseThumbnails =
              this.appOptions.canUseViwerNavigation =
                isPDFViewer;
            this.appOptions.canDownloadForms =
              this.appOptions.canLicense && this.appOptions.canDownload;
            ...
            this.appOptions.canFavorite =
              ...
              !this.appOptions.isOffline;
            this.appOptions.canFavorite &&
              appHeader.setFavorite(this.document.info.favorite);

            this.appOptions.canUseReviewPermissions =
              this.appOptions.canLicense &&
              (!!this.permissions.reviewGroups ||
                (this.editorConfig.customization &&
                  this.editorConfig.customization.reviewPermissions &&
            ...
            AscCommon.UserInfoParser.setCurrentName(
              this.appOptions.user.fullname
            );
            this.appOptions.canUseReviewPermissions &&
              AscCommon.UserInfoParser.setReviewPermissions(
                this.permissions.reviewGroups,
                this.editorConfig.customization.reviewPermissions
              );
            this.appOptions.canUseCommentPermissions &&
              AscCommon.UserInfoParser.setCommentPermissions(
                this.permissions.commentGroups
              );
            this.appOptions.canUseUserInfoPermissions &&
              AscCommon.UserInfoParser.setUserInfoPermissions(
                this.permissions.userInfoGroups
              );
            appHeader.setUserName(
              AscCommon.UserInfoParser.getParsedName(
                AscCommon.UserInfoParser.getCurrentName()
              )
            );
            ...
            this.editorConfig.customization &&
              Common.UI.LayoutManager.init(
                this.editorConfig.customization.layout,
                this.appOptions.canBrandingExt
              );
            this.editorConfig.customization &&
            ...
            this.appOptions.canLiveView =
              ...
              this.editorConfig.mode === "view" &&
              !isPDFViewer; // viewer: change=false when no flag canLiveViewer (i.g. old license), change=true by default when canLiveViewer==true
            ...
            this.appOptions.isAnonymousSupport =
              !!this.api.asc_isAnonymousSupport();
          ...
          loadCoAuthSettings: function () {
            ...
            if (
              ...
              if (!this.appOptions.canChangeCoAuthoring) {
                //can't change co-auth. mode. Use coEditing.mode or 'fast' by default
                value =
                  this.editorConfig.coEditing &&
                  this.editorConfig.coEditing.mode !== undefined
                    ? this.editorConfig.coEditing.mode === "strict"
                      ? 0
                      : 1
                    : null;
                if (
                  value === null &&
                  this.appOptions.customization &&
                  this.appOptions.customization.autosave === false
                ) {
                ...
                if (value === null) {
                  ...
                  if (
                    value === null &&
                    !Common.localStorage.itemExists("de-settings-autosave") &&
                    this.appOptions.customization &&
                    this.appOptions.customization.autosave === false
                  ) {
              ...
              value = Common.localStorage.getItem(
                fastCoauth
                  ? "de-settings-showchanges-fast"
                  : "de-settings-showchanges-strict"
              );
              ...
              // viewer
              value = Common.localStorage.getItem(
                "de-settings-view-coauthmode"
              );
              if (!this.appOptions.canChangeCoAuthoring || value === null) {
                // Use coEditing.mode or 'fast' by default
                value =
                  this.editorConfig.coEditing &&
                  this.editorConfig.coEditing.mode === "strict"
                    ? 0
          ...
          applyModeCommonElements: function () {
            ...
            this.api.asc_registerCallback(
              "asc_onDownloadUrl",
              _.bind(this.onDownloadUrl, this)
            );
            this.api.asc_registerCallback(
            ...
            value === undefined &&
              (value = Common.Utils.Metric.getDefaultMetric());
          ...
          applyModeEditorElements: function () {
            ...
            var me = this,
              ...
              reviewController = application.getController(
                "Common.Controllers.ReviewChanges"
              );
            reviewController
              .setMode(me.appOptions)
            ...
            if (this.appOptions.isRestrictedEdit)
              ...
              var viewport = this.getApplication()
                .getController("Viewport")
                .getView("Viewport");
              ...
              var toolbarView = toolbarController
              ...
              value = Common.localStorage.itemExists("de-hidden-rulers")
                ? Common.localStorage.getBool("de-hidden-rulers")
                : this.appOptions.customization &&
                  !!this.appOptions.customization.hideRulers;
              Common.Utils.InternalSettings.set("de-hidden-rulers", value);
              me.api.asc_SetViewRulers(!value);

              me.api.asc_registerCallback(
                "asc_onDocumentCanSaveChanged",
                _.bind(me.onDocumentCanSaveChanged, me)
              );
              /** coauthoring begin **/
              me.api.asc_registerCallback(
                "asc_onCollaborativeChanges",
                _.bind(me.onCollaborativeChanges, me)
              );
              me.api.asc_registerCallback(
              ...
              me.api.asc_registerCallback(
                "asc_onConvertEquationToMath",
                _.bind(me.onConvertEquationToMath, me)
              );
              /** coauthoring end **/

              if (
                me.stackLongActions.exist({
                  id: ApplyEditRights,
                  type: Asc.c_oAscAsyncActionType["BlockInteraction"],
                })
              ) {
          ...
          onExternalMessage: function (msg, options) {
            if (msg && msg.msg) {
              ...
              this.showTips(
                [msg.msg.charAt(0).toUpperCase() + msg.msg.substring(1)],
                options
              );
          ...
          onError: function (id, level, errData) {
            if (id == Asc.c_oAscError.ID.LoadingScriptError) {
              ...
              this.showTips([this.errorCannotPasteImg], {
                timeout: 7000,
                hideCloseTip: true,
              });
              return;
            ...
            switch (id) {
              ...
              case Asc.c_oAscError.ID.SplitCellMaxRows:
                config.msg = this.splitMaxRowsErrorText.replace(
                  "%1",
                  errData.get_Value()
                );
                break;
              ...
              case Asc.c_oAscError.ID.SplitCellMaxCols:
                config.msg = this.splitMaxColsErrorText.replace(
                  "%1",
                  errData.get_Value()
                );
                break;
              ...
              case Asc.c_oAscError.ID.SplitCellRowsDivider:
                config.msg = this.splitDividerErrorText.replace(
                  "%1",
                  errData.get_Value()
                );
                break;
              ...
              default:
                config.msg =
                  typeof id == "string"
                    ? id
                    : this.errorDefaultMessage.replace("%1", id);
                break;
            }
            ...
            if (level == Asc.c_oAscError.Level.Critical) {
              ...
              config.callback = _.bind(function (btn) {
                if (
                  ...
                  id == Asc.c_oAscError.ID.SplitCellMaxRows ||
          ...
          onDocumentModifiedChanged: function () {
            ...
            if (this._state.isDocModified !== isModified) {
              this._isDocReady &&
                Common.Gateway.setDocumentModified(
            ...
            var toolbarView = this.getApplication()
              .getController("Toolbar")
              .getView();
            if (
              toolbarView &&
              ...
              var isSyncButton =
                ...
                forcesave = this.appOptions.forcesave,
                isDisabled =
                  (!isModified && !isSyncButton && !forcesave) ||
                  this._state.isDisconnected ||
                  (this._state.fastCoauth &&
                    this._state.usersCount > 1 &&
                    !forcesave);
          ...
          onBeforeUnload: function () {
            ...
            } else this._state.unloadTimer = 10000;
          },
              ...
              if (this.appOptions.canBrandingExt) {
                ...
                if (
                  ...
                  if (this.appOptions.customization.rightMenu !== undefined)
                    console.log(
                      "Obsolete: The 'rightMenu' parameter of the 'customization' section is deprecated. Please use 'rightMenu' parameter in the 'customization.layout' section instead."
                    );
          ...
          fillAutoShapes: function (groupNames, shapes) {
            if (
              _.isEmpty(shapes) ||
              _.isEmpty(groupNames) ||
              shapes.length != groupNames.length
            )
              return;
          ...
          onAdvancedOptions: function (type, advOptions, mode, formatOptions) {
            ...
            if (type == Asc.c_oAscAdvancedOptionsID.TXT) {
              me._state.openDlg = new Common.Views.OpenDialog({
                title: Common.Views.OpenDialog.prototype.txtTitle.replace(
                  "%1",
                  "TXT"
                ),
                ...
                handler: function (result, settings) {
                  ...
                  if (result == "ok") {
                    if (me && me.api) {
                      if (mode == 2) {
                        formatOptions &&
                          formatOptions.asc_setAdvancedOptions(
                            settings.textOptions
                          );
                        me.api.asc_DownloadAs(formatOptions);
                      } else
                        me.api.asc_setAdvancedOptions(
                          type,
                          settings.textOptions
                        );
              ...
              me._state.openDlg = new Common.Views.OpenDialog({
                ...
                warning:
                  !(me.appOptions.isDesktopApp && me.appOptions.isOffline) &&
          ...
          onTryUndoInFastCollaborative: function () {
            if (!Common.localStorage.getBool("de-hide-try-undoredo"))
              Common.UI.info({
                width: 500,
                msg: this.appOptions.canChangeCoAuthoring
                  ? this.textTryUndoRedo
                  : this.textTryUndoRedoWarn,
                iconCls: "info",
                buttons: this.appOptions.canChangeCoAuthoring
                  ? ["custom", "cancel"]
                  : ["ok"],
                ...
                callback: _.bind(function (btn, dontshow) {
                  if (dontshow)
                    Common.localStorage.setItem("de-hide-try-undoredo", 1);
                  if (btn == "custom") {
          ...
          onUserConnection: function (change) {
            if (
              ...
              if (name && name !== AscCommon.UserInfoParser.getCurrentName()) {
                ...
                appHeader.setUserName(
                  AscCommon.UserInfoParser.getParsedName(name)
                );

                var idx1 = name.lastIndexOf("("),
                  idx2 = name.lastIndexOf(")"),
                  str =
                    idx1 > 0 && idx1 < idx2 ? name.substring(0, idx1 - 1) : "";
          ...
          applySettings: function () {
            if (
              ...
              this._state.fastCoauth = Common.localStorage.getBool(
                "de-settings-coauthmode",
                true
              );
          ...
          onMeta: function (meta) {
            ...
            var filemenu = this.getApplication()
              .getController("LeftMenu")
              .getView("LeftMenu")
              .getMenu("file");
            filemenu.loadDocument({ doc: this.document });
            filemenu.panels &&
              filemenu.panels["info"] &&
            ...
            if (this.appOptions.wopi) {
              ...
              Common.Gateway.requestRename(
                idx > 0 ? meta.title.substring(0, idx) : meta.title
              );
            }
          },
              ...
              this.iframePrint.onload = function () {
                try {
                  ...
                  me.api.asc_DownloadAs(
                    new Asc.asc_CDownloadOptions(Asc.c_oAscFileType.PDF)
                  );
                }
              };
            }
          ...
          DisableMailMerge: function () {
            ...
            var toolbarController =
              this.getApplication().getController("Toolbar");
          ...
          onConvertEquationToMath: function (equation) {
            var me = this,
              win;
            var msg =
              this.textConvertEquation +
              '<br><br><a id="id-equation-convert-help" style="cursor: pointer;">' +
              this.textLearnMore +
              "</a>";
            ...
            win.$window
              ...
              .on("click", function (e) {
                ...
                me.getApplication()
                  ...
                  .showMenu(
                    "file:help",
                    "UsageInstructions/InsertEquation.htm#convertequation"
                  );
              });
          },
          ...
          onRunAutostartMacroses: function () {
            ...
            if (enable) {
              ...
              if (value == 1) this.api.asc_runAutostartMacroses();
              else if (value === 0) {
                Common.UI.warning({
          ...
          onMacrosPermissionRequest: function (url, callback) {
            ...
            var me = this;
            var value = Common.Utils.InternalSettings.get(
              "de-allow-macros-request"
            );
            if (value > 0) {
          ...
          loadAutoCorrectSettings: function () {
            // autocorrection
            var me = this;
            var value = Common.localStorage.getItem(
              "de-settings-math-correct-add"
            );
            Common.Utils.InternalSettings.set(
              "de-settings-math-correct-add",
              value
            ...
            Common.Utils.InternalSettings.set(
              "de-settings-math-correct-rem",
              value
            );
            var arrRem = value ? JSON.parse(value) : [];
            value = Common.localStorage.getBool(
              "de-settings-math-correct-replace-type",
              true
            ); // replace on type
            Common.Utils.InternalSettings.set(
              "de-settings-math-correct-replace-type",
            ...
            me.api.asc_refreshOnStartAutoCorrectMathSymbols(
              arrRem,
              arrAdd,
              value
            );
            ...
            value = Common.localStorage.getBool(
              "de-settings-autoformat-bulleted",
              true
            );
            Common.Utils.InternalSettings.set(
              "de-settings-autoformat-bulleted",
            ...
            value = Common.localStorage.getBool(
              "de-settings-autoformat-numbered",
              true
            );
            Common.Utils.InternalSettings.set(
              "de-settings-autoformat-numbered",
            ...
            value = Common.localStorage.getBool(
              "de-settings-autoformat-smart-quotes",
              true
            );
            Common.Utils.InternalSettings.set(
              "de-settings-autoformat-smart-quotes",
            ...
            value = Common.localStorage.getBool(
              "de-settings-autoformat-hyphens",
              true
            );
            Common.Utils.InternalSettings.set(
              "de-settings-autoformat-hyphens",
            ...
            value = Common.localStorage.getItem(
              "de-settings-letter-exception-sentence"
            );
            value =
              value !== null
                ? parseInt(value) != 0
                : Common.localStorage.getBool(
                    "de-settings-autoformat-fl-sentence",
                    true
                  );
            Common.Utils.InternalSettings.set(
              "de-settings-letter-exception-sentence",
            ...
            value = Common.localStorage.getItem(
              "de-settings-letter-exception-cells"
            );
            value =
              value !== null
                ? parseInt(value) != 0
                : Common.localStorage.getBool(
                    "de-settings-autoformat-fl-cells",
                    true
                  );
            Common.Utils.InternalSettings.set(
              "de-settings-letter-exception-cells",
            ...
            [0x0409, 0x0419].forEach(function (lang) {
              var apiFlManager = me.api
                .asc_GetAutoCorrectSettings()
                .get_FirstLetterExceptionManager();

              value = Common.localStorage.getItem(
                "de-settings-letter-exception-add-" + lang
              );
              Common.Utils.InternalSettings.set(
                "de-settings-letter-exception-add-" + lang,
                value
              ...
              value = Common.localStorage.getItem(
                "de-settings-letter-exception-rem-" + lang
              );
              Common.Utils.InternalSettings.set(
                "de-settings-letter-exception-rem-" + lang,
                value
            ...
            value = Common.localStorage.getBool(
              "de-settings-autoformat-hyperlink",
              true
            );
            Common.Utils.InternalSettings.set(
              "de-settings-autoformat-hyperlink",
            ...
            value = Common.localStorage.getBool(
              "de-settings-autoformat-double-space",
              Common.Utils.isMac
            ); // add period with double-space in MacOs by default
            Common.Utils.InternalSettings.set(
              "de-settings-autoformat-double-space",
              value
            );
          ...
          showRenameUserDialog: function () {
            ...
            this._renameDialog = new Common.Views.UserNameDialog({
              ...
              check:
                Common.Utils.InternalSettings.get("save-guest-username") ||
                false,
              ...
              handler: function (result, settings) {
                if (result == "ok") {
                  var name = settings.input
                    ? settings.input + " (" + me.appOptions.guestName + ")"
                    : me.textAnonymous;
                  var _user = new Asc.asc_CUserInfo();
          ...
          leavePageText:
            "You have unsaved changes in this document. Click 'Stay on this Page' then 'Save' to save them. Click 'Leave this Page' to discard all the unsaved changes.",
          ...
          requestEditFailedMessageText:
            "Someone is editing this document right now. Please try again later.",
          txtNeedSynchronize: "You have an updates",
          textLoadingDocument: "Loading document",
          warnBrowserZoom:
            "Your browser's current zoom setting is not fully supported. Please reset to the default zoom by pressing Ctrl+0.",
          warnBrowserIE9:
            "The application has low capabilities on IE9. Use IE10 or higher",
          applyChangesTitleText: "Loading Data",
          applyChangesTextText: "Loading data...",
          errorKeyEncrypt: "Unknown key descriptor",
          errorKeyExpire: "Key descriptor expired",
          errorUsersExceed: "Count of users was exceed",
          errorCoAuthoringDisconnect:
            "Server connection lost. You can't edit anymore.",
          errorFilePassProtect:
          ...
          warnProcessRightsChange:
            "You have been denied the right to edit the file.",
          ...
          errorDatabaseConnection:
            "External error.<br>Database connection error. Please, contact support.",
          ...
          txtYAxis: "Y Axis",
          txtSeries: "Seria",
          errorMailMergeLoadFile:
            "Loading the document failed. Please select a different file.",
          ...
          errorConnectToServer:
            "The document could not be saved. Please check connection settings or contact your administrator.<br>When you click the 'OK' button, you will be prompted to download the document.",
          textTryUndoRedo:
          ...
          errorViewerDisconnect:
            "Connection is lost. You can still view the document,<br>but will not be able to download or print until the connection is restored and page is reloaded.",
          warnLicenseExp:
          ...
          errorToken:
            "The document security token is not correctly formed.<br>Please contact your Document Server administrator.",
          errorTokenExpire:
            "The document security token has expired.<br>Please contact your Document Server administrator.",
          errorSessionAbsolute:
            "The document editing session has expired. Please reload the page.",
          errorSessionIdle:
            "The document has not been edited for quite a long time. Please reload the page.",
          errorSessionToken:
            "The connection to the server has been interrupted. Please reload the page.",
          errorAccessDeny:
            "You are trying to perform an action you do not have rights for.<br>Please contact your Document Server administrator.",
          ...
          txtNoTableOfContents:
            "There are no headings in the document. Apply a heading style to the text so that it appears in the table of contents.",
          ...
          warnNoLicense:
            "You've reached the limit for simultaneous connections to %1 editors. This document will be opened for viewing only.<br>Contact %1 sales team for personal upgrade terms.",
          warnNoLicenseUsers:
            "You've reached the user limit for %1 editors. Contact %1 sales team for personal upgrade terms.",
          warnLicenseExceeded:
          ...
          warnLicenseUsersExceeded:
            "You've reached the user limit for %1 editors. Contact your administrator to learn more.",
          errorDataEncrypted:
            "Encrypted changes have been received, they cannot be deciphered.",
          textClose: "Close",
          textPaidFeature: "Paid feature",
          scriptLoadError:
            "The connection is too slow, some of the components could not be loaded. Please reload the page.",
          errorEditingSaveas:
            "An error occurred during the work with the document.<br>Use the 'Save as...' option to save the file backup copy to your computer hard drive.",
          errorEditingDownloadas:
            "An error occurred during the work with the document.<br>Use the 'Download as...' option to save the file backup copy to your computer hard drive.",
          ...
          txtShape_flowChartMagneticTape:
          ...
          txtShape_accentBorderCallout1:
            "Line Callout 1 (Border and Accent Bar)",
          txtShape_accentBorderCallout2:
            "Line Callout 2 (Border and Accent Bar)",
          txtShape_accentBorderCallout3:
          ...
          txtShape_curvedConnector3WithTwoArrows:
          ...
          textCustomLoader:
            "Please note that according to the terms of the license you are not entitled to change the loader.<br>Please contact our Sales Department to get a quote.",
          txtHyperlink: "Hyperlink",
          waitText: "Please, wait...",
          errorFileSizeExceed:
            "The file size exceeds the limitation set for your server.<br>Please contact your Document Server administrator for details.",
          ...
          errorUpdateVersionOnDisconnect:
            "Internet connection has been restored, and the file version has been changed.<br>Before you can continue working, you need to download the file or copy its content to make sure nothing is lost, and then reload this page.",
          txtChoose: "Choose an item",
          errorDirectUrl:
            "Please verify the link to the document.<br>This link must be a direct link to the file for downloading.",
          txtStyle_Caption: "Caption",
          errorCompare:
            "The Compare documents feature is not available in the co-editing mode.",
          textConvertEquation:
            "This equation was created with an old version of equation editor which is no longer supported. Converting this equation to Office Math ML format will make it editable.<br>Do you want to convert this equation?",
          textApplyAll: "Apply to all equations",
          textLearnMore: "Learn More",
          txtEnterDate: "Enter a date",
          txtTypeEquation: "Type equation here",
          textHasMacros:
            "The file contains automatic macros.<br>Do you want to run macros?",
          textRemember: "Remember my choice for all files",
          warnLicenseLimitedRenewed:
            "License needs to be renewed.<br>You have a limited access to document editing functionality.<br>Please contact your administrator to get full access",
          warnLicenseLimitedNoAccess:
            "License expired.<br>You have no access to document editing functionality.<br>Please contact your administrator.",
          saveErrorTextDesktop:
            "This file cannot be saved or created.<br>Possible reasons are: <br>1. The file is read-only. <br>2. The file is being edited by other users. <br>3. The disk is full or corrupted.",
          errorComboSeries:
            "To create a combination chart, select at least two series of data.",
          ...
          leavePageTextOnClose:
            "All unsaved changes in this document will be lost.<br> Click 'Cancel' then 'Save' to save them. Click 'OK' to discard all the unsaved changes.",
          textTryUndoRedoWarn:
            "The Undo/Redo functions are disabled for the Fast co-editing mode.",
          txtNone: "None",
          ...
          errorLoadingFont:
            "Fonts are not loaded.<br>Please contact your Document Server administrator.",
          errorEmptyTOC:
            "Start creating a table of contents by applying a heading style from the Styles gallery to the selected text.",
          errorNoTOC:
            "There's no table of contents to update. You can insert one from the References tab.",
          textRequestMacros:
            "A macro makes a request to URL. Do you want to allow the request to the %1?",
          textRememberMacros: "Remember my choice for all macros",
          errorTextFormWrongFormat:
            "The value entered does not match the format of the field.",
          errorPasswordIsNotCorrect:
            "The password you supplied is not correct.<br>Verify that the CAPS LOCK key is off and be sure to use the correct capitalization.",
          confirmMaxChangesSize:
            'The size of actions exceeds the limitation set for your server.<br>Press "Undo" to cancel your last action or press "Continue" to keep action locally (you need to download the file or copy its content to make sure nothing is lost).',
          textUndo: "Undo",
          ...
          errorInconsistentExt:
            "An error has occurred while opening the file.<br>The file content does not match the file extension.",
          errorCannotPasteImg:
          ...
          textTryQuickPrint:
            "You have selected Quick print: the entire document will be printed on the last selected or default printer.<br>Do you want to continue?",
          textAnyone: "Anyone",
          textText: "Text",
          warnLicenseBefore:
            "License not active.<br>Please contact your administrator.",
          titleLicenseNotActive: "License not active",
          warnLicenseAnonymous:
            "Access denied for anonymous users. This document will be opened for viewing only.",
        };
      })(),
      DE.Controllers.Main || {}
    )
